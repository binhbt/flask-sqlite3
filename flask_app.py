...
sys.path.insert(1,
                "PATH TO LOCAL PYTHON PACKAGES")  # OPTIONAL: Only if need to access Python packages installed on a local (non-global) directory
...
def sql_datainsert():
    ...
    if request.method == 'POST':
        ...
        sql_edit_insert(''' INSERT INTO words (word,payload,created_time) VALUES (?,?,?) ''',
...
def sql_dataedit():
    ...
    if request.method == 'POST':
        ...
        sql_edit_insert(
            ''' UPDATE data_table set first_name=?,last_name=?,address=?,city=?,state=?,zip=? WHERE first_name=? and last_name=? ''',
